...
namespace Infection
{
    ...
    /// </summary>
    public class TimeDelayInfection : MonoBehaviour
    {
        /// <summary>
        ...
        /// Based on if the humans is infecitious and if the latency period has passed
        /// </summary>
        ...
        public bool IsInfectious()
        {
            ...
            if (myHuman.GetCondition() == HumanBase.INFECTIOUS)
                return true;
            // if I do not show symptoms yet, I only infect others after the latency period
            else if (myHuman.GetCondition() == HumanBase.EXPOSED && Time.fixedTime - myHuman.t_start_incubation >= t_latent)
                return true;
            else
                return false;
        }
        ...
        public void Infect()
        {
            if (myHuman.IsSusceptible() && UnityEngine.Random.value < InfectionRate)
            {
